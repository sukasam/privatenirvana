<?php 
	...
	$page_name = "(News Gallery ".get_news_name($conn,$_GET['news_id']).")";
	$tbl_name = "s_news_gallery";
	$fieldlist = array('news_id','file_name','sorts','flag_show') ;
	$title_del="News Gallery";
	$title_del_name="img_id";
	//$a_param = array('page','orderby','sortby','keyword','pagesize','news_id','mid','smid');
	//$pagesize = 5;
...
<UL class=shortcut-buttons-set> 
	...
	?>
  <LI><A class=shortcut-button href="list.php?mode=add&news_id=<?php  echo $_GET["news_id"]; if($param <> "") {?>&<?php  echo $param; }?>"><SPAN><IMG  alt=icon src="../images/btn_back.gif"><BR>Back</SPAN></A></LI>
  <?php  }?> 
</UL>
...
<div class="tab-content default-tab" id="tab1" style="display: block;"><!-- This is the target div. id must match the href of this div's tab -->
    ...
    {
    var settings = {
        url: "upload.php?news_id=<?php  echo $_GET['news_id'];?>&news_name=<?php  echo get_news_name($conn,$_GET['news_id']);?>",
        dragDrop:true,
        fileName: "myfile",
        ...
        //returnType:"json",
         onSuccess:function(files,data,xhr){
           var data2 = {
               mode:"resize",
			   news_id: "<?php  echo $_GET['news_id'];?>",
			   news_name: "<?php  echo get_news_name($conn,$_GET['news_id']);?>",
			   fileName: data,
           }
           $.ajax({
					'type':'POST',
        ...
        {
            $.post("delete.php?news_id=<?php  echo $_GET['news_id'];?>",{op:"delete",name:data[i]},
